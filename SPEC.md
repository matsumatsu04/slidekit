...
- **デザインは4種（`data-hstyle="a"` `"b"` `"c"` `"f"`）。見た目だけが変わり、上記metricsはどのスタイルでも変えない：**
...
| F | ドット＋英字ラベル | 上段に**小さいアクセント色の丸ドット＋英字ラベル**（12px・bold、`data-label`属性の文言）、その下に日本語タイトル（24px・bold） |

- **D（2トーン下線）と E（ショートバー）は 2026-08-19 に廃止した**（代表指示）。`patterns/` からCSSを削除し、
  ビルダーの `HEADING_STYLES` からも外してある。ビルド済みのデッキはCSSを焼き込んであるため表示は変わらないが、
  `headingStyle` に `d`/`e` を書いたデッキを**再ビルドするとエラーで止まる**（該当2本＝`output/nol-ai-deck-v2`、
...
- 実装は `patterns/` 側は `.sk-head`/`.sk-head-ttl` ＋ `data-hstyle` 属性切替（sk-head v5）、
  ...
  Fスタイルの英字ラベル文言は、`patterns/*.html` では `.sk-head` の `data-label` 属性、
