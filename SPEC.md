...
| B | 縦バー | タイトル左に**アクセント縦バー**（幅4px・左から40pxの位置）。下線なし |
...
| F | タブ型 | タイトルが**角丸のアクセント色タブ**（左下だけ角なし）に乗り、白文字で表示 |

- 実装は `patterns/` 側は `.sk-head`/`.sk-head-ttl` ＋ `data-hstyle` 属性切替（sk-head v4）、
...
## バージョン
- SPEC version: 1.5（共通見出しをデザインA/Bの2種からA〜Fの6種へ拡張）
