...
> 構図パターンライブラリ（`patterns/SLIDE-PATTERN-*`）は [sho-ai-magic/slide.md](https://github.com/sho-ai-magic/slide.md)（MIT License）を
...
カテゴリは 12 種：`表紙・セクション / 目次 / コンテンツ（本文） / テキストリスト / フロー・ステップ /
図解・ダイアグラム / カード・グリッド / グラフ・データ / テーブル・比較 / KPI・まとめ / Q&A・FAQ / プロフィール`。
...
- `category` は上記 12 種から選ぶ（INDEX とギャラリーの分類に使う）。
...
- 既存ライブラリ（sho-ai-magic/slide.md, MIT）は内容・ライセンス表示を保持したまま利用する（[CREDITS](./patterns/CREDITS.md)）。
...
## バージョン
- SPEC version: 1.1（構図パターンを `SLIDE-PATTERN-{name}` 形式・`patterns/` ライブラリに統一）
