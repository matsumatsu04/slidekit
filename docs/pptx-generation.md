...
**デザインシステムとAI一般則が矛盾する場合はデザインシステムを優先する**
（例: clean-mono-sansの「見出し下の全幅アクセント下線」は仕様なので入れる）。
...
QA画像は **デッキレビューア** で通し確認できる（ローカルサーバー起動時）:
`/gallery/deck-review.html?base=<画像パス>/slide&count=<枚数>`（またはページに画像をドラッグ&ドロップ）。
