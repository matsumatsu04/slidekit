...
画像があれば抽出モード、なければ設計モード。`category` を 12 種から決める：
`表紙・セクション / 目次 / コンテンツ（本文） / テキストリスト / フロー・ステップ / 図解・ダイアグラム /
カード・グリッド / グラフ・データ / テーブル・比較 / KPI・まとめ / Q&A・FAQ / プロフィール`。
...
- INDEX と manifest.json に追記済み（カテゴリは 12 種のいずれか）。
